...
function predict() {
  ...
  posenetModel.estimateSinglePose(video).then((pose) => {
    ...
    if (frameCount === frame) {
      ...
      numScore.innerHTML = Math.round(angleDegrees);
      cntScore++;
    ...
    if (shouldStop) {
      ...
      const avgWristScore = 40;
      const finalScore = calculatePostureScore(basicNackScore, basicWristScore, avgNackScore, avgWristScore);
...
// 함수: 기준 데이터 수신
function getBasicScore(endPoint){
